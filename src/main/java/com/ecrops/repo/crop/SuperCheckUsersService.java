...
@Transactional
@Component
public class SuperCheckUsersService {
	...
	public List<McodeMnameSuperCheck> getMcodeAndMname(String wbdcode, String season, String activeyear, int cropyear,
		...
		query.setParameter(3, userid);// System.out.println(" bst1:"+st1);
	...
	@Transactional
	public List<Object> getData(int cropyear, String dcode, int mcode, String userid, String season) {

		if (dcode.length() == 1) {
			dcode = "0" + dcode;
		}

		String tab = "ecrop" + cropyear + ".cr_crop_det_new_v_" + season + dcode + cropyear;
		...
		query.setParameter(3, Integer.parseInt(dcode));
	...
	public List<AuthorityVerifyReasons> getcodeAndreason() {
		...
		for (Object[] ob : resultList) {
			Object[] data = (Object[]) ob; // Assuming each object in the result is an array with two elements
			int code = (int) data[0];
	...
	public List<CrCropDetNewV> checkAndViewHomeMandalViewData(int cropyear, String wbdcode, int wbmcode, String userid,
		...
		List<CrCropDetNewV> entities = null;

		if (cropyear < 0) {
			throw new IllegalArgumentException("Crop year cannot be negative.");
		}

		if (wbdcode == null || wbdcode.isEmpty()) {
			throw new IllegalArgumentException("WBD code cannot be null or empty.");
		}

		if (wbmcode < 0) {
			throw new IllegalArgumentException("WBM code cannot be negative.");
		}
		...
		if (vcode == null || vcode.isEmpty()) {
			throw new IllegalArgumentException("Vcode cannot be null or empty.");
		}

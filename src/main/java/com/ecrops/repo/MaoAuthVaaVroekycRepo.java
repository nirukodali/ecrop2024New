package com.ecrops.repo;

import java.util.List; 

import org.springframework.data.jpa.repository.JpaRepository;
import org.springframework.data.jpa.repository.Query;

import com.ecrops.entity.MaoAuthVaaVroekyc;
import com.ecrops.projection.MasterProjections;

public interface MaoAuthVaaVroekycRepo extends JpaRepository<MaoAuthVaaVroekyc, Integer> {
	@Query(value="select distinct on (a.cropyear,a.season) concat(a.season,'@',cropyear) as col1,concat(b.seasonname,' ',cropyear) as col2 \r\n" + 
			" from activeseason a,season b  where   a.season=b.season and a.active='A' order by a.cropyear,a.season",nativeQuery = true)
	public List<com.ecrops.projection.MasterProjections> getAllSeason();
	
	@Query(value="select cropid as col1,cropname as col2 from cropnames  where active='A' order by cropname",nativeQuery = true)
	public List<com.ecrops.projection.MasterProjections> getAllCrops();
	
	@Query(value = "select cropid as col1,cropnameeng as col2 from cropnames  where active='A' order by cropnameeng", nativeQuery = true)
	public List<MasterProjections> getAllCropeng();

}

...
public interface WbvillagesRepository extends JpaRepository<WbVillageMastEntity, Integer> {
	...
	@Query(value="select wbvcode, wbvname from wbvillage_mst where wbvcode in(select vcode from ecrop2024.villsec_rev_v where vscode=:rbkcode)",nativeQuery=true)
	public List<WbVillageMastProjection> getWebLandDet(@Param("rbkcode") Integer rbkCode);
	
	@Query(value="select wbvcode, wbvname from wbvillage_mst where mcode=:mcode and ecrop_dwn='Y' and wbvcode not in (select vcode from ecrop2024.downloaddetails where cropyear=:cropyear and season=:season)",nativeQuery=true)
